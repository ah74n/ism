...
function App() {
  ...
  const analyzeURL = async () => {
    ...
    try {
      // Pointing to your Railway backend or local server
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL || "https://is-malicious-production-b4dc.up.railway.app"}/api/scan-url`,
        { url: url }
      );
      ...
      console.error(err);
      setError(
        err.response?.data?.error || err.response?.data?.detail || "Backend connection failed. Server might be sleeping."
      );
    } finally {
      setLoading(false);
    }
